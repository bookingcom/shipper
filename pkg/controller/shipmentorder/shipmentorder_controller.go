...
// Controller is the controller implementation for ShipmentOrder resources
...
func NewController(
	...
	shipmentOrderInformer := shipperInformerFactory.Shipper().V1().ShipmentOrders()
	...
	controller := &Controller{
		...
		shipmentOrdersLister: shipmentOrderInformer.Lister(),
		shipmentOrdersSynced: shipmentOrderInformer.Informer().HasSynced,
		workqueue:            workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "ShipmentOrders"),
		recorder:             recorder,
	}

	glog.Info("Setting up event handlers")
	// Set up an event handler for when ShipmentOrder resources change
	shipmentOrderInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	glog.Info("Starting ShipmentOrder controller")

	// Wait for the caches to be synced before starting workers
	glog.Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.shipmentOrdersSynced); !ok {
		return fmt.Errorf("failed to wait for caches to sync")
	}

	glog.Info("Starting workers")
	// Launch two workers to process ShipmentOrder resources
...
func (c *Controller) processNextWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		// ShipmentOrder resource to be synced.
...
func (c *Controller) syncHandler(key string) error {
	...
	// Get the ShipmentOrder resource with this namespace/name
	so, err := c.shipmentOrdersLister.ShipmentOrders(namespace).Get(name)
	if err != nil {
		// The ShipmentOrder resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			runtime.HandleError(fmt.Errorf("ShipmentOrder '%s' in work queue no longer exists", key))
			return nil
		}
	...
	// Finally, we update the status block of the ShipmentOrder resource to reflect the
	...
	c.recorder.Event(so, corev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	return nil
}

// enqueueShipmentOrder takes a ShipmentOrder resource and converts it into a namespace/name
// string which is then put onto the work queue. This method should *not* be
// passed resources of any type other than ShipmentOrder.
