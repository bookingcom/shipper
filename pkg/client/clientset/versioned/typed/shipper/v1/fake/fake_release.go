...
func (c *FakeReleases) Get(name string, options v1.GetOptions) (result *shipper_v1.Release, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(releasesResource, c.ns, name), &shipper_v1.Release{})

	if obj == nil {
		return nil, err
	}
	return obj.(*shipper_v1.Release), err
}

// List takes label and field selectors, and returns the list of Releases that match those selectors.
func (c *FakeReleases) List(opts v1.ListOptions) (result *shipper_v1.ReleaseList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(releasesResource, releasesKind, c.ns, opts), &shipper_v1.ReleaseList{})
...
func (c *FakeReleases) Create(release *shipper_v1.Release) (result *shipper_v1.Release, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(releasesResource, c.ns, release), &shipper_v1.Release{})
...
func (c *FakeReleases) Update(release *shipper_v1.Release) (result *shipper_v1.Release, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(releasesResource, c.ns, release), &shipper_v1.Release{})
...
func (c *FakeReleases) UpdateStatus(release *shipper_v1.Release) (*shipper_v1.Release, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(releasesResource, "status", c.ns, release), &shipper_v1.Release{})
...
func (c *FakeReleases) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(releasesResource, c.ns, name), &shipper_v1.Release{})
...
func (c *FakeReleases) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	...
	_, err := c.Fake.Invokes(action, &shipper_v1.ReleaseList{})
	return err
}

// Patch applies the patch and returns the patched release.
func (c *FakeReleases) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *shipper_v1.Release, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(releasesResource, c.ns, name, data, subresources...), &shipper_v1.Release{})
