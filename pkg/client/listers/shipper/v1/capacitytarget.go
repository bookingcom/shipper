...
type CapacityTargetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CapacityTarget, err error)
...
// List lists all CapacityTargets in the indexer.
func (s *capacityTargetLister) List(selector labels.Selector) (ret []*v1.CapacityTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CapacityTarget))
	})
	return ret, err
}
...
type CapacityTargetNamespaceLister interface {
	...
	Get(name string) (*v1.CapacityTarget, error)
...
// List lists all CapacityTargets in the indexer for a given namespace.
func (s capacityTargetNamespaceLister) List(selector labels.Selector) (ret []*v1.CapacityTarget, err error) {
...
// Get retrieves the CapacityTarget from the indexer for a given namespace and name.
func (s capacityTargetNamespaceLister) Get(name string) (*v1.CapacityTarget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("capacitytarget"), name)
	}
	return obj.(*v1.CapacityTarget), nil
}
