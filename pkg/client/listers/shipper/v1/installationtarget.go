...
type InstallationTargetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.InstallationTarget, err error)
...
// List lists all InstallationTargets in the indexer.
func (s *installationTargetLister) List(selector labels.Selector) (ret []*v1.InstallationTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.InstallationTarget))
	})
	return ret, err
}
...
type InstallationTargetNamespaceLister interface {
	...
	Get(name string) (*v1.InstallationTarget, error)
...
// List lists all InstallationTargets in the indexer for a given namespace.
func (s installationTargetNamespaceLister) List(selector labels.Selector) (ret []*v1.InstallationTarget, err error) {
...
// Get retrieves the InstallationTarget from the indexer for a given namespace and name.
func (s installationTargetNamespaceLister) Get(name string) (*v1.InstallationTarget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("installationtarget"), name)
	}
	return obj.(*v1.InstallationTarget), nil
}
