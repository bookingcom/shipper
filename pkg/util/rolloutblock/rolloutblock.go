...
func ShouldOverride(overrideRB string, rbs []*shipper.RolloutBlock) (bool, string, error) {
	overrideRBs := strings.Split(overrideRB, ",")
	if len(overrideRB) == 0 {
		overrideRBs = []string{}
	}

	nonOverriddenRBs, err := Diff(rbs, overrideRBs)
...
func Diff(existingRBs []*shipper.RolloutBlock, overrideRBs []string) ([]string, error) {
	var diff []string
